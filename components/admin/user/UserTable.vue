...
<template>
    <div>
        <div class="w-full h-10 flex justify-between items-center cursor-pointer">
            ...
            <div
                class="flex items-center gap-2 border-2 rounded-lg px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-blue-500 bg-white">
                <SearchIcon class="w-5 h-5 text-gray-500 scale-x-[-1]" />
                <input v-model="props.userStore.keyWordSearch" @input="props.userStore.onSearch" type="text"
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr v-for="(item, index) in props.store.data?.data" :key="item.id ?? index"
                        ...
                        <td class="px-6 py-4">
                            {{
                                ((props.store.data?.meta?.per_page ?? 0) * ((props.store.data?.meta?.current_page ?? 1) -
                                    1)) + index + 1
                            }}
                        </td>
